...
//     }
//
//     // Simulating judge behavior by adding a submission entry with 'Pending' verdict
//     const submission = await db.submission.create({
//       data: {
//         userId: user_id,
//         problemId: problem_id,
//         sourceCode: source_code,
//         verdict: "Pending", // Verdict is set to Pending
//       },
//     }).catch((error) => {
//       console.error('Database error:', error);
//       throw new Error('Failed to save submission');
...
export async function POST(req: NextRequest) {
  try {
    const body = await req.json();
    let { language_id, problem_id, source_code, user_id } = body;

    if (!language_id) language_id = 73;
    if (!problem_id) problem_id = "unknown-problem";
    if (!source_code) source_code = "// Default source code\nconsole.log('Hello, World!');";
    if (!user_id) user_id = "vaurn.ai@gmail.com";

    // Save the submission with default values
    const submission = await db.submission.create({
      data: {
        userId: user_id,
        problemId: problem_id,
    ...
    return NextResponse.json({
      submission_id: submission.id,
      verdict: "Pending",
      message: "Submission recorded with default values where necessary.",
    });
  } catch (error) {
    console.error("Submission error:", error);
    return NextResponse.json(
